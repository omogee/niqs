...
class LabouroutputForm extends Component {
    constructor(props) {
        super(props);
        this.state = { 
            category:"",
...
formData.append("file",this.state.csv);
        axios.post(`https://niqsdatabank.herokuapp.com/api/v1/labouroutput/import`,formData,config)
        .then(res => {
    ...
    submit=()=>{
        ...
        axios.post(`https://niqsdatabank.herokuapp.com/api/v1/labouroutput`,data,config)
        .then(res =>{ 
            if(res.data.message && res.data.data){
            this.setState({messagebox:res.data.message,messageboxclass:"alert-success"},()=>{
         this.setState({category:"",description:"",subcategory:"",output:""})
         setTimeout(()=>this.setState({messagebox:"",messageboxclass:"d-none"}),5000)
        })
    }else{
       this.setState({redirect:true})
        ...
        return ( 
            <div className="container">
                ...
                <div className="col-12 mt-5">
                 <h2 className="text-primary">LABOUR OUTPUT</h2>
                 </div>                
             ...
             <div className="col-sm-12 col-md-6 mb-3">
                        Category : 
                        <input type="text" name="category" className="form-control" onChange={this.change}/>
                    </div>
                    <br/>
                    <div className="col-sm-12 col-md-6 mb-3">
                        Sub-Category : 
                        <input type="text" name="subcategory" className="form-control" onChange={this.change}/>
                    </div>
                    <div className="col-sm-12 col-md-6 mb-3">
                       description : 
                        <textarea name="description" cols="3" rows="3" className="form-control" onChange={this.change}></textarea>
                    </div>
                    <div className="col-sm-12 col-md-6 mb-3">
                        Output : 
                        <input type="text" name="output" onChange={this.change} className="form-control"/>
                    </div>
...
export default LabouroutputForm;